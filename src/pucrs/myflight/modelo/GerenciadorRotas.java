...
public class GerenciadorRotas {
    ...
    public void carregaDados(String nomeArq, GerenciadorAeronaves gerAvioes,
                             GerenciadorAeroportos gerAero, GerenciadorCias gerCias) throws IOException {
        Path path = Paths.get(nomeArq);
        try (Scanner sc = new Scanner(Files.newBufferedReader(path, Charset.forName("utf8")))) {
            sc.useDelimiter("[;\n]"); // separadores: ; e nova linha
            String header = sc.nextLine(); // pula cabeçalho
            String origemstr, destinostr, aviaostr, ciastr; int x;

            while (sc.hasNext()) {
                aviaostr = sc.next();
                gerAvioes.buscarCodigo(aviaostr);
                origemstr = sc.next();
                gerAero.buscarCodigo(origemstr);
                destinostr = sc.next();
                gerAero.buscarCodigo(destinostr);
                x = sc.nextInt();
                ciastr = sc.next();
                gerCias.buscarCodigo(ciastr);
